...
public class InvoiceMapperImpl implements InvoiceMapper {
    ...
    public InvoiceEntity toEntity(InvoiceDTO source) {
        ...
        if ( source.getIssued() != null ) {
            invoiceEntity.setIssued( Date.from( source.getIssued().atStartOfDay( ZoneOffset.UTC ).toInstant() ) );
        }
        if ( source.getDueDate() != null ) {
            invoiceEntity.setDueDate( Date.from( source.getDueDate().atStartOfDay( ZoneOffset.UTC ).toInstant() ) );
        }
    ...
    public InvoiceDTO toDTO(InvoiceEntity source) {
        ...
        if ( source.getIssued() != null ) {
            invoiceDTO.setIssued( LocalDateTime.ofInstant( source.getIssued().toInstant(), ZoneOffset.UTC ).toLocalDate() );
        }
        if ( source.getDueDate() != null ) {
            invoiceDTO.setDueDate( LocalDateTime.ofInstant( source.getDueDate().toInstant(), ZoneOffset.UTC ).toLocalDate() );
        }
    ...
    @Override
    public InvoiceDTO toDTO(PersonEntity source) {
        if ( source == null ) {
            return null;
        }
